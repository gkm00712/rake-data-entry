...
yesterday_ist = today_ist - timedelta(days=1) # Restricting to Yesterday and Today

# --- SESSION STATE INITIALIZATION ---
if 'rake_val' not in st.session_state:
    st.session_state.rake_val = ""
if 'outages_list' not in st.session_state:
    st.session_state.outages_list = [] # Stores multiple outages for the table
...
with tab1:
    ...
    # --- 2. DEPARTMENTAL OUTAGES (MULTIPLE ENTRY + TABLE) ---
    st.subheader("🛠️ Departmental Outages")
    st.caption("Fill the details and press Enter (or click Add) to compile them into the table below.")
    
    # We use a mini-form here so pressing 'Enter' adds the outage without submitting the whole rake
    with st.form("outage_form", clear_on_submit=True):
        ...
        with d4:
            ...
            add_outage = st.form_submit_button("➕ Add Outage")
            
        # Logic to add to our table
        if add_outage and o_start:
            if not o_end:
                log_str = f"[{dept}: FULL DAY OUTAGE (Start: {o_start})]"
                display_end = "FULL DAY"
            else:
                log_str = f"[{dept}: {o_start} to {o_end}]"
                display_end = o_end
                
            st.session_state.outages_list.append({
                "Department": dept, 
                "Start Time": o_start, 
                "End Time": display_end, 
                "Compiled Log": log_str
            })

    # Show the table if data exists
    if st.session_state.outages_list:
        st.write("**Current Outages Recorded for this Rake:**")
        df_outages = pd.DataFrame(st.session_state.outages_list)
        # Display a clean table without the hidden Compiled Log column
        st.table(df_outages[["Department", "Start Time", "End Time"]])
        
        if st.button("🗑️ Clear Outages List"):
            st.session_state.outages_list.clear()
            st.rerun()

    st.divider()

    # --- 3. TIMELINE (YESTERDAY/TODAY + 1-MINUTE RESOLUTION) ---
    st.subheader("📅 Timeline (IST - Restricted to Yesterday/Today)")
    ...
    st.caption("Format Example: '33 07:10-11:40'")
    ...
    # The General remarks text area comes below the Outages
    ...
    if st.button("🚀 Submit Final Data to Master Sheet", type="primary"):
        # Validate Rake Format
        if not re.match(r"^\d+/\d+$", st.session_state.rake_val):
            st.error("❌ Error: RAKE No must be XX/XXXX (e.g., 1/1481). Entry rejected.")
            st.session_state.rake_val = ""
            st.rerun()
        else:
            # Compile all outages from the table into one string
            outage_summary = "\n".join([o["Compiled Log"] for o in st.session_state.outages_list])
            
            # Combine the Table Outages + General Remarks for Column L
            ...
            with st.spinner("Writing securely to Google Sheets..."):
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.balloons()
                        # Clear the outages table for the next rake
                        st.session_state.outages_list.clear()
